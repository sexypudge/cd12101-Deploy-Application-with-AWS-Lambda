...
export function Todos() {
  ...
  function renderTodosList() {
    return (
      <Grid padded>
        {todos.filter(todo => todo !== undefined).map((todo, pos) => {
          return (
  ...
  async function onTodoDelete(todoId) {
    try {
      const accessToken = await getAccessTokenSilently({
        audience: `https://dev-6glb826logh3h5ks.us.auth0.com/api/v2/`,
        scope: 'delete:todo'
      })
  ...
  useEffect(() => {
    ...
    foo()
  }, [getAccessTokenSilently, loadingTodos])

  return (
    <div>
      <Header as="h1">TODOs</Header>

      <NewTodoInput onNewTodo={(newTodo) => setTodos([...todos, newTodo])} onSetReload={() => setLoadingTodos(true)} />
